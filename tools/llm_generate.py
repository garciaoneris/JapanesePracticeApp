...
SYSTEM_PROMPT = """You are a Japanese-language content generator that writes short natural example sentences for language learners.
Respond ONLY with JSON matching the schema. No prose, no markdown, no explanations.

Rules for each sentence:
- sentence_jp MUST contain the target word (exact substring match).
- The ONLY kanji allowed in sentence_jp are those in allowed_kanji. Every other character must be hiragana, katakana, or standard Japanese punctuation (。 、 ? !).
- segs: tokenize sentence_jp so concatenating every seg.t reproduces sentence_jp exactly (same characters, same order).
- For any seg whose t contains any kanji, r MUST be the hiragana reading of that full token (never katakana, never romaji, never with dots/dashes).
- g is a 1-3 word English gloss, set ONLY on content words (nouns, verbs, adjectives). Leave g null on particles, auxiliaries, copulas, and punctuation.
- Keep sentence_jp short and natural: under 25 Japanese characters, everyday vocabulary."""
...
async def call_vllm(
    ...
    output via guided_json. Returns the raw JSON string in message.content."""
...
def validate_example(
    ...
    # 1. Target word must appear in the sentence.
    if word_jp not in ex.sentence_jp:
        return f"sentence_jp must contain the word {word_jp!r}"

    # 2. Segs must concatenate to the sentence exactly.
...
def word_level(kanji_chars: list[str], kanji_index: Mapping[str, Any]) -> int:
    ...
    words get level 1 (easiest) so they can use only Lvl 1 kanji if
    they introduce any in the example."""
    if not kanji_chars:
        return 1
